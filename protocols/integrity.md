...
- From S02 onward, one active `BL-` profile resolves, is user-selected and verified, pins an exact baseline contract, and every active candidate/commitment has the same baseline ID/contract version and a bounded delta statement.
...
- Baseline-specific search is only one branch: task, mechanism, and failure-mode prior-art coverage remains explicit.
