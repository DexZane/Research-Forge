...
### 4. 用机制签名比较创新，用承诺版本保留审计链

每篇需要比较的关键文献和每个候选都会记录一个[创新机制签名](protocols/innovation-signature.md)：瓶颈、操作、被改变的对象、关键条件和可观察对比。这样审稿人式的问题就变得具体：即使标题或实现不同，最近的工作是否已经覆盖了同一条机制？

尚未核验的历史提示会记录为 `AWARENESS_ONLY` 的 `AL-` 线索。它们只能指导检索，不能成为引用、证据、新颖性支持或 BibTeX 条目。每个候选还拥有版本化的 `CM-` 核心承诺；一旦机制、预测、证伪条件或预算发生语义变化，[承诺完整性协议](protocols/commitment-integrity.md) 就会创建可追溯的新版本，并强制重新核验受影响的新颖性映射、测试、可行性估计、审稿意见和 Gate。
...
只有经过去重、`verification_status: VERIFIED`、必需字段齐全、存在权威来源或稳定标识符、且没有未解决元数据冲突的记录，才会导出。临时记录、仅来自搜索摘要的结果，以及所有 `AWARENESS_ONLY` 线索都会留在项目 registry 中，不会进入引用文件。
